...
export default function StockManagementScreen() {
  ...
  const loadBatches = async (product: InventoryItem) => {
    ...
    try {
      ...
      const payload = (await response.json()) as { batches?: StockBatch[]; error?: string };
  ...
  const submitAction = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/commands/stock-management', {
        ...
        body: JSON.stringify({ action, productId: selectedProduct.id, quantity: parsedQuantity, unitCost: Number(unitCost), sellingPrice: sellingPrice ? Number(sellingPrice) : undefined, expiryDate, sku, barcode, supplier, batchId, reason, notes }),
      });
  ...
  return (
    <div className="mx-auto w-full max-w-screen-2xl space-y-4 p-3 sm:p-5 lg:p-6">
      ...
      <section className="rounded-xl border border-border bg-card p-4 shadow-card sm:p-5"><div className="flex items-center gap-2"><History size={16} className="text-primary" /><h2 className="text-sm font-semibold">Batch & SKU History</h2></div>{selectedProduct ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead><tr className="border-b border-border text-[10px] uppercase tracking-wider text-muted-foreground"><th className="px-2 py-2">SKU / Barcode</th><th className="px-2 py-2">Received</th><th className="px-2 py-2">Remaining</th><th className="px-2 py-2">Cost</th><th className="px-2 py-2">Expiry</th><th className="px-2 py-2">Status</th></tr></thead><tbody className="divide-y divide-border">{loadingBatches ? <tr><td colSpan={6} className="px-2 py-6 text-center text-muted-foreground">Loading batches...</td></tr> : batches.length ? batches.map((batch) => <tr key={batch.id}><td className="px-2 py-3"><span className="font-medium">{batch.sku}</span><span className="block text-muted-foreground">{batch.barcode || '—'}</span></td><td className="px-2 py-3 tabular-nums">{batch.quantityReceived}</td><td className="px-2 py-3 tabular-nums">{batch.quantityRemaining}</td><td className="px-2 py-3">{formatMoney(batch.unitCost, settings.currency)}</td><td className="px-2 py-3">{batch.expiryDate}</td><td className="px-2 py-3 capitalize">{batch.status}</td></tr>) : <tr><td colSpan={6} className="px-2 py-6 text-center text-muted-foreground">No receipt batches recorded yet.</td></tr>}</tbody></table></div> : <p className="mt-3 text-sm text-muted-foreground">Select a product to view its receipt and SKU history.</p>}</section>
